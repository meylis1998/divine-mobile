...
void main() {
  group('AuthService Event Creation', () {
    test('should create Kind 0 event with 5-minute backdated timestamp', () async {
      ...
      print('Testing Kind 0 (profile) event timestamp:');
      print('Current UTC time: $currentTime (${NostrTimestamp.format(currentTime)})');
      print('Kind 0 timestamp: $kind0Timestamp (${NostrTimestamp.format(kind0Timestamp)})');
      print('Backdate amount: ${currentTime - kind0Timestamp} seconds (${(currentTime - kind0Timestamp) / 60} minutes)');
    ...
    test('should create Kind 22 event with 30-second backdated timestamp', () async {
      final currentTime = DateTime.now().toUtc().millisecondsSinceEpoch ~/ 1000;
      final kind22Tolerance = NostrTimestamp.getDriftToleranceForKind(22);
      final kind22Timestamp = NostrTimestamp.now(driftTolerance: kind22Tolerance);
      
      print('Testing Kind 22 (video) event timestamp:');
      print('Current UTC time: $currentTime (${NostrTimestamp.format(currentTime)})');
      print('Kind 22 timestamp: $kind22Timestamp (${NostrTimestamp.format(kind22Timestamp)})');
      print('Backdate amount: ${currentTime - kind22Timestamp} seconds');
